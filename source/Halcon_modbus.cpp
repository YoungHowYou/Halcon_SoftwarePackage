...
Herror Hmodbus_rtu_connect(Hproc_handle proc_handle)
{
	...
	(*pUserData)->modbusCtx = modbus_new_rtu(device.par.s, (int)baud.par.l, (char)parity.par.l, (int)data_bit.par.l, (int)stop_bit.par.l);
...
Herror Hmodbus_read_bits(Hproc_handle proc_handle)
{
	...
	int64_t *C;
	HAllocTmp(proc_handle, &C, AddresssLength.par.l * sizeof(int64_t));
	for (size_t i = 0; i < AddresssLength.par.l; i++)
	{
		C[i] = (int64_t)Bits_Besult[i];
	}
...
Herror Hmodbus_read_inputbits(Hproc_handle proc_handle)
{
	Hcpar Addressstrat;	  // 起始地址
	Hcpar AddresssLength; // 读取数据长度
	...
	int Get_Bits_Besult = modbus_read_input_bits(pUserData->modbusCtx, Addressstrat.par.l, AddresssLength.par.l, Bits_Besult);
...
Herror Hmodbus_read_registers(Hproc_handle proc_handle)
{
	...
	else
	{
		...
		int64_t *C;
		HAllocTmp(proc_handle, &C, Length.par.l * sizeof(int64_t));
		for (size_t i = 0; i < Length.par.l; i++)
		{
			C[i] = (int64_t)Read_Registers_Num[i];
		}
...
Herror Hmodbus_read_register_float(Hproc_handle proc_handle)
{
	...
	else
	{
		...
		if (strcmp(EncodingMode.par.s, "abcd") == 0)
		{
			C = (double)modbus_get_float_abcd(Read_Registers_Num);
		}
		else if (strcmp(EncodingMode.par.s, "dcba") == 0)
		{
			C = (double)modbus_get_float_dcba(Read_Registers_Num);
		}
		else if (strcmp(EncodingMode.par.s, "badc") == 0)
		{
			C = (double)modbus_get_float_badc(Read_Registers_Num);
		}
		else if (strcmp(EncodingMode.par.s, "cdab") == 0)
		{
			C = (double)modbus_get_float_cdab(Read_Registers_Num);
		}
		else
		{
			C = (double)modbus_get_float(Read_Registers_Num);
		}
...
Herror Hmodbus_read_register_int(Hproc_handle proc_handle)
{
	...
	else
	{
		...
		{
			int64_t val = (int64_t)MODBUS_GET_INT64_FROM_INT16(regs64, 0);
